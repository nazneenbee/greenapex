...
public interface IEmployeeRepository extends JpaRepository<EmployeeEntity, Integer>{
	...
	@Query("select new com.greenapex.dto.HighestSalaryEmployeeDetail(e.empName,e.empSalary) from EmployeeEntity e where e.empSalary IN(Select max(e.empSalary) FROM EmployeeEntity e JOIN DepartmentEntity d ON e.department.deptId = d.deptId group by d.deptId)")
